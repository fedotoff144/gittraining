...
CMD_EXIT = 'e'
MULTIPLICITY = 50
PERCENTAGE_4_WITHDRAWAL = decimal.Decimal(15) / decimal.Decimal(
    1000)  # some explanation
MIN_FEE = 30
MAX_FEE = 600
COUNT_OPERATION = 3
...
while True:
    cmd = input(
        f'Enter command ({CMD_REPLENISH} = replenish, '
        f'{CMD_WITHDRAW} = withdraw, {CMD_EXIT} = exit:\n')
    if cmd == CMD_EXIT:
    ...
    if cmd in (CMD_REPLENISH, CMD_WITHDRAW):
        ...
        if cmd == CMD_WITHDRAW:
            ...
            if balance > withdrawal_amount:
                ...
                print(
                    f'Transaction amount = {amount}, withdrawal commission = '
                    f'{withdrawal_fee}, balance = {balance}')
                count += 1
            else:
                print(
        ...
        if count % COUNT_OPERATION == 0:
            ...
            print(
                f'You have performed three operations. '
                f'Your bonus is {bonus}. Balance is '
                f'{balance}')
